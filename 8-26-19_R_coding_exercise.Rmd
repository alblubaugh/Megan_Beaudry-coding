...
#we want to load the dslabs package
...
#use the help() fuctnion to see what the data contains
##the dataset contains health and incomeoutcomes for 184 coutnies from 1960 to 2016
#str - displays basic strucutual info. The nine variables of this dataset are: country, year, infant_mortality, life_expectany, fertility, population, gdp, continent, region
#summary - statistical summaries of selected data. Type of data shown depends on class
```{r}
help(gapminder)
str(gapminder)
summary(gapminder)
...
#*PROCESSING DATA*
#using base R notation assing only african countries to the variable africadata
## <- right of the chosen variable name 
###all african counties are accessed through the dataset gapminder by using the variable content. The accessor $ is used to access specific variables (i.e., continent)
####we then pull out the african counties by using a logical vector (i.e., ==) to determine if the variable content is listed as "Africa"
```{r}
africadata <- gapminder$continent == "Africa"
```

#str - basic information on the variable
#summary - statistical summaries of selected data
```{r}
str(africadata)
summary(africadata)
```
##the results from this code chunk indicated that we have 2907 arguments that are "true" out of a total of 10545
###this results in our dataset being dereased to 2908

#Now we want to define two new variables: (1) Infant_LifeExpec_Africa ~ which includes infant_mortality and life_expectency and (2) Pop_LifeExp_Africa ~ includes population and life_expectancy
...
#We need to define the pieces of the dataframe by using equal signs followed by the variables we created earlier 
```{r}
